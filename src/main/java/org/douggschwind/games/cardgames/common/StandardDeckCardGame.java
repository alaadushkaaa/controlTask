...
public abstract class StandardDeckCardGame {
	...
	protected final Map<Kind, Integer> determineNumKindOccurrencesInHand(List<FrenchSuitedPlayingCard> playersHand) {
		Map<Kind, Integer> result = new HashMap<>();
	...
	protected final int determineMaximumNumberMatchingKind(Map<Kind, Integer> numKindOccurrences) {
	...
	protected final Map<Player, List<Kind>> getPlayerSortedDistinctCardKindsMap(Set<Player> players) {
		Map<Player, List<Kind>> result = new HashMap<>();
	...
	protected final Kind determineDominantMatchingKind(Map<Kind, Integer> numKindOccurrences, int numDominantMatchesExpected) {
		Optional<Kind> foundCardKind = numKindOccurrences.keySet().stream().filter(cardKind -> numKindOccurrences.get(cardKind) == numDominantMatchesExpected).findFirst();
		return foundCardKind.map(kind -> kind).orElse(null);
	}
	
	protected final Pair determinePlayerHandPair(Map<Kind, Integer> numKindOccurrences) {
		Optional<Kind> foundCardKind = numKindOccurrences.keySet().stream().filter(cardKind -> numKindOccurrences.get(cardKind) == 2).findFirst();
	...
	private HandStrength determinePlayerHandStrengthNoMatchingKinds(Player player) {
		...
		int lowToHighDistance = Kind.computeDistance(lowCard.getKind(), highCard.getKind());
		if (numSuitOccurrences.size() == 1) {
			...
			if (lowToHighDistance == 4) {
				...
				lowToHighDistance = Kind.computeDistance(lowCard.getKind(), secondHighestCard.getKind());
				if ((lowToHighDistance == 3) &&
					(lowCard.getKind() == Kind.Two)) {
	...
	protected final HandStrength determinePlayerHandStrengthThreeDistinctKindsInHand(int maxNumberMatchingKind, Map<Kind, Integer> numKindOccurrences) {
		if (maxNumberMatchingKind == 3) {
			...
			List<Kind> pairedKinds = numKindOccurrences.keySet().stream().filter(cardKind -> numKindOccurrences.get(cardKind) == 2).collect(Collectors.toList());
			Kind pair1 = pairedKinds.get(0);
			Kind pair2 = pairedKinds.get(1);
	...
	public HandStrength determinePlayerHandStrength(Player player) {
		Map<Kind, Integer> numKindOccurrences = determineNumKindOccurrencesInHand(player.getHand());
	 ...
	 * @return
	 */
	private Set<Kind> getDistinctGameCommonCardKinds(Map<Player, List<Kind>> playerSortedDistinctCardKindsMap) {
	 ...
	 * @return
	 */
	private Map<Player, List<Kind>> getCommonEliminatedSortedPlayerCardKindsMap(Set<Player> players) {
		Map<Player, List<Kind>> result = getPlayerSortedDistinctCardKindsMap(players);
		Set<Kind> distinctGameCommonCardKinds = getDistinctGameCommonCardKinds(result);
	...
	public final Set<Player> determineWinners() {
		...
		Map<Player, List<Kind>> playerSortedDistinctCardKindsMap = getCommonEliminatedSortedPlayerCardKindsMap(leadingPlayers);
		Player aLeadingPlayer = playerSortedDistinctCardKindsMap.keySet().iterator().next();
		List<Kind> aLeadingPlayerUniqueDistinctCardKinds = playerSortedDistinctCardKindsMap.get(aLeadingPlayer);
		...
		for (int cardIndex = 0;cardIndex < aLeadingPlayerUniqueDistinctCardKinds.size();cardIndex++) {
			Kind cardKindLeadingAtIndex = null;
			...
			for (Player player : playerSortedDistinctCardKindsMap.keySet()) {
				...
				List<Kind> uniqueDistinctPlayerCardKinds = playerSortedDistinctCardKindsMap.get(player);
